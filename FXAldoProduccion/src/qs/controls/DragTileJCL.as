...
package qs.controls
{
	...
	public class DragTileJCL extends Container
	{
		...
		override protected function commitProperties():void
		{
			...
			if(renderersDirty)
			{
				...
				{
					var renderer:UIComponent = _renderCache.associate(_items[i]);
					
					IDataRenderer(renderer).data = _items[i];
					
					renderer.opaqueBackground = 2;
			...
			if(renderers.length > 0)
			{
				if(isNaN(_userTileHeight) || isNaN(_userTileWidth))
				{
					for(var i:int=0;i<renderers.length;i++)
					{
						...
						//HABILITAR PARA TOOLTIP itemRenderer.toolTip = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
		...
		private function startReorder(e:MouseEvent):void
		{
			...
			var dragItem:UIComponent = renderers[dragIdx];
		...
		{
			this.document.dragAction.text=dragAction;
			_dragAction = dragAction;
			DragManager.showFeedback(_dragAction);				
		...
		private function dragDrop(e:DragEvent):void
		{
			...
			else
			{
				...
				else
				{
					...
					else if (e.dragSource.hasFormat("itemsByIndex"))
					{
						var draggedItems:*=e.dragSource.dataForFormat("itemsByIndex ");
					}
					/////////////////////////////////////////
					if(e.target.id=="DataGrid"){
						this.document.rosterDestino.text = "-1";
					}else{
						this.document.rosterDestino.text = e.target.id;
		...
		{
			var r:UIComponent = _itemRendererFactory.newInstance();
		...
		{
			//////////////////////////////////////////
			if(e.currentTarget.id=="DataGrid"){
				this.document.rosterOrigen.text = "-1";
			}else{
				this.document.rosterOrigen.text = e.currentTarget.id;
				this.document.execInsUpdXMLXArea.text="1";
			}
			...
			if(e.action == DragManager.MOVE && e.dragSource.dataForFormat("target") != this)
			{
				...
				{
					_items.splice(dragFromIndex,1);
					var r:UIComponent = renderers.splice(dragFromIndex,1)[0];
		...
		private function generateLayout():void
		{
			...
			{			
				...
				if(idx >= renderers.length)
					return;								
				var renderer:UIComponent = renderers[idx];
